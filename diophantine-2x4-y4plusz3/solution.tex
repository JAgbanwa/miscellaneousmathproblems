...
\title{%
  ...
  \large Six Parametric Families with Lean~4 Formalisation
}
\author{%
...
\begin{theorem}[Main result]\label{thm:main}
  ...
  In particular, each of the following six parametric families provides infinitely many
  ...
  \begin{align}
    ...
    \text{Family 5:} &\quad (x,y,z) = (196t^3,\; 392t^3,\; 2744t^4), && t \in \Z,\label{eq:fam5}\\
    \text{Family 6:} &\quad (x,y,z) = (1922t^3,\; 961t^3,\; -29791t^4), && t \in \Z.\label{eq:fam6}
  \end{align}
\end{theorem}
...
\begin{remark}
  ...
  Up to these sign symmetries, the six families are the only ones found by exhaustive
  search for $|x|, |y| \le 2500$ (Families~1--5 appear within $|x|,|y|\le 500$;
  Family~6 appears at $|x|,|y|\le 2500$; no additional primitive seed is found).
\end{remark}
...
\begin{remark}
  ...
  immediately gives infinitely many.  The six primitive seeds
  $(0,1,1)$, $(1,1,-1)$, $(4,0,-8)$, $(14,21,49)$, $(196,392,2744)$, and
  $(1922,961,-29791)$ each generate a distinct infinite parametric family under
  this scaling.
\end{remark}
...
\subsection{Family 5: \texorpdfstring{$(196t^3,\, 392t^3,\, 2744t^4)$}{(196t³,392t³,2744t⁴)}}

Set $y = 2x$.  Equation~\eqref{eq:main} becomes
\[
  2x^4 - (2x)^4 + z^3 = 2x^4 - 16x^4 + z^3 = -14x^4 + z^3 = 0,
\]
so $z^3 = 14x^4 = 2\cdot 7\cdot x^4$.  Writing $x = 2^a\cdot 7^b\cdot m^3$, the
right-hand side is $2^{4a+1}\cdot 7^{4b+1}\cdot m^{12}$.  For this to be a perfect
cube we need $4a+1\equiv 0\pmod 3$ and $4b+1\equiv 0\pmod 3$, giving $a\equiv b\equiv 2\pmod 3$.
The minimal positive solution is $a=b=2$, $m=t$, yielding:
\[
...
\begin{remark}[Key identity for Family~5]
  \[
    2\cdot 4^4 - 8^4 + 8^3\cdot 7 = 512 - 4096 + 3584 = 0.
  \]
  Multiplying by $7^8$:
  \[
    2(4\cdot 7^2)^4 - (8\cdot 7^2)^4 + (8\cdot 7^3)^3
    = 7^8(512 - 4096 + 3584) = 0,
  \]
  i.e.\ $2\cdot 196^4 - 392^4 + 2744^3 = 0$.
\end{remark}

\noindent Substituting: $2(196t^3)^4 - (392t^3)^4 + (2744t^4)^3 = t^{12}(2\cdot 196^4 - 392^4 + 2744^3) = 0$. $\checkmark$

\subsection{Family 6: \texorpdfstring{$(1922t^3,\, 961t^3,\, -29791t^4)$}{(1922t³,961t³,-29791t⁴)}}

Set $x = 2p^2$, $y = p^2$, $z = -p^3$ for an integer $p$.  Substituting:
\[
  2(2p^2)^4 - (p^2)^4 + (-p^3)^3
  = 2\cdot 16p^8 - p^8 - p^9
  = p^8(32 - 1 - p)
  = p^8(31 - p).
\]
This equals zero if and only if $p = 0$ or $p = 31$.  Taking $p = 31$:
\[
  x = 2\cdot 31^2 = 1922,\quad y = 31^2 = 961,\quad z = -31^3 = -29791.
\]

\begin{remark}[Key identity for Family~6]
  \[
    2\cdot 2^4 - 1^4 - 31 = 32 - 1 - 31 = 0.
  \]
  Multiplying by $31^8$:
  \[
    2(2\cdot 31^2)^4 - (31^2)^4 + (-31^3)^3
    = 31^8(32 - 1 - 31) = 0,
  \]
  i.e.\ $2\cdot 1922^4 - 961^4 + (-29791)^3 = 0$.
\end{remark}

\begin{remark}
  The factor $(31 - p)$ vanishes only at $p = 31$, so this seed does not arise
  from a two-parameter algebraic substitution.  The infinite parametric family
  $(1922t^3, 961t^3, -29791t^4)$ is produced by weighted scaling of the primitive
  seed $(1922, 961, -29791)$.
\end{remark}

\noindent Substituting: $2(1922t^3)^4 - (961t^3)^4 + (-29791t^4)^3 = t^{12}(2\cdot 1922^4 - 961^4 - 29791^3) = 0$. $\checkmark$
...
$(196, 392, 2744)$ & $2\cdot196^4$ & $-392^4$ & $2744^3$ & $0$ \\
$(-196, -392, 2744)$ & $2\cdot196^4$ & $-392^4$ & $2744^3$ & $0$ \\
$(1922, 961, {-}29791)$ & $2\cdot1922^4$ & $-961^4$ & $(-29791)^3$ & $0$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent
(Family~5 at $t=1$: $2\cdot 196^4 - 392^4 + 2744^3 = 7^8(512-4096+3584)=0$.
 Family~6 at $t=1$: $2\cdot 1922^4 - 961^4 + (-29791)^3 = 31^8(32-1-31)=0$.)
...
-- Family 5: (196*t^3, 392*t^3, 2744*t^4)
-- Core identity: 2*4^4 - 8^4 + 8^3*7 = 512 - 4096 + 3584 = 0
theorem family5 (t : Int) : isolution (196 * t ^ 3) (392 * t ^ 3) (2744 * t ^ 4) := by
  unfold isolution; ring

-- Family 6: (1922*t^3, 961*t^3, -29791*t^4)
-- Core identity: 2*2^4 - 1^4 - 31 = 32 - 1 - 31 = 0
theorem family6 (t : Int) : isolution (1922 * t ^ 3) (961 * t ^ 3) (-(29791 * t ^ 4)) := by
  unfold isolution; ring
...
\midrule
\texttt{weightedHomogeneity} & Scaling $(n^3x, n^3y, n^4z)$ is a solution & \texttt{linear\_combination} & $\checkmark$ fully proved \\
\texttt{family1} & $(0, t^3, t^4)$ is a solution & \texttt{ring} & $\checkmark$ fully proved \\
\texttt{family2} & $(t^3, t^3, -t^4)$ is a solution & \texttt{ring} & $\checkmark$ fully proved \\
\texttt{family3} & $(4t^3, 0, -8t^4)$ is a solution & \texttt{ring} & $\checkmark$ fully proved \\
\texttt{family4} & $(14t^3, 21t^3, 49t^4)$ is a solution & \texttt{ring} & $\checkmark$ fully proved \\
\texttt{family5} & $(196t^3, 392t^3, 2744t^4)$ is a solution & \texttt{ring} & $\checkmark$ fully proved \\
\texttt{family6} & $(1922t^3, 961t^3, -29791t^4)$ is a solution & \texttt{ring} & $\checkmark$ fully proved \\
\texttt{sol\_14\_21\_49} & Seed solution $(14, 21, 49)$ & \texttt{norm\_num} & $\checkmark$ fully proved \\
\texttt{sol\_196\_392\_2744} & Seed solution $(196, 392, 2744)$ & \texttt{norm\_num} & $\checkmark$ fully proved \\
\texttt{sol\_1922\_961\_neg29791} & Seed solution $(1922, 961, -29791)$ & \texttt{norm\_num} & $\checkmark$ fully proved \\
\texttt{pow3\_strictMono} & $n \mapsto n^3$ strictly mono on $\N$ & \texttt{Nat.pow\_lt\_pow\_left} & $\checkmark$ fully proved \\
\texttt{natMap\_injective} & $n \mapsto (0, n^3, n^4)$ is injective & \texttt{pow3\_strictMono} & $\checkmark$ fully proved \\
\texttt{solutions\_infinite} & Solution set is infinite & \texttt{Set.infinite\_range} & $\checkmark$ fully proved \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
  \item \textbf{Complete classification.}  Are the six families (and their sign variants)
    ...
    a descent argument or an effective height bound on the six rational curves
    $\mathcal{C}_1, \ldots, \mathcal{C}_6$ tracing the six families.
  \item \textbf{Are there further primitive seeds?}  Exhaustive search finds no sporadic
    solution within $|x|, |y| \le 2500$.  Whether another primitive seed exists at
    larger bounds is an open computational and number-theoretic question.
  \item \textbf{The minimal-prime identity.}  Family~4 rests on $2\cdot 2^4 - 3^4 + 7^2 = 0$,
    and Family~6 rests on $2\cdot 2^4 - 1 - 31 = 0$ (unique since $2\cdot 2^4 - 1 - p = 0$
    has the sole solution $p = 31$).  Are there other integer triples $(a, b, c)$ giving
    $2a^4 - b^4 + c^3 = 0$ with $\gcd(a,b,c)$ ``small'' relative to the magnitude?
\end{enumerate}
