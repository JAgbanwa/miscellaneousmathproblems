...
\begin{table}[h]
\centering
\caption{Integer solutions to $x + x^2 y^2 + z^3 = 0$ with $|x|,|y|,|z| \leq 10{,}000$.}
\label{tab:solutions}
\begin{tabular}{rrrll}
\toprule
...
(where $Y = y$ and $X = z$).  Siegel's theorem (1929) guarantees finitely many integer points
on any elliptic curve; Baker's method (1968) makes the bound effective and computable.
...
\midrule
\texttt{family1} & $0 + 0^2 n^2 + 0^3 = 0$ & \texttt{ring} \\
