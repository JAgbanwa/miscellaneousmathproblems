...
\title{%
  ...
  \large Parametric Families and Computational Evidence, with Lean~4 Formalisation
}
\author{%
...
\subsection{Computational search for primitive solutions}

\begin{remark}[Computational evidence only]\label{rem:search}
  A brute-force search over $|x|, |y| \leq 500$ finds the following
  primitive (i.e.\ $\gcd = 1$) solutions and no others within that range:

  \begin{table}[h]
  \centering
  \caption{Primitive solutions found within $|x|,|y|\leq 500$ (computational search).}
  \label{tab:primitive}
  \renewcommand{\arraystretch}{1.2}
  \begin{tabular}{rrrl}
  \toprule
  ...
  \textbf{Important caveat.}  A finite search is not a proof of completeness.
  The above is computational evidence, not a theorem.  In particular, it does
  \emph{not} rule out the existence of primitive solutions with $|x| > 500$
  or $|y| > 500$ that lie outside all three known families.  The full
  classification of primitive solutions remains an open problem.
\end{remark}

\begin{conjecture}\label{conj:prim}
  The only primitive solutions to $x^5 + y^4 = 2z^2$ are, up to the
  symmetries $y \to -y$ and $z \to -z$:
  \[
    (1,\;1,\;1) \quad \text{and} \quad (-1,\;1,\;0).
  \]
  A proof would require either a descent argument, a factorisation in
  $\Z[\sqrt{2}]$ or a suitable number field, or a height-bound argument
  from arithmetic geometry.
\end{conjecture}

% ════════════════════════════════════════════════════════════════════════════
\section{Structural Observations}\label{sec:structure}

\subsection{Weighted projective motivation}

\begin{remark}[Heuristic context --- not a rigorous argument]
The equation~\eqref{eq:main} is weighted-homogeneous of degree~$20$ with
weight vector $(4,5,10)$.  This motivates searching for solutions by
scaling a seed: if $(x_0,y_0,z_0)$ satisfies the equation, so does
$(t^4 x_0, t^5 y_0, t^{10} z_0)$.  The parametric families in this paper
arise exactly this way.  We mention weighted projective space
$\mathbb{P}^2_{(4,5,10)}$ only as motivating language; we do not invoke
any theorem about integral points on weighted projective varieties, and no
result in this paper depends on this framing.
\end{remark}

\subsection{Remark on Faltings' theorem}

Faltings' theorem concerns smooth projective \emph{curves} of genus $\geq 2$.
The solution set of~\eqref{eq:main} is two-dimensional (it is a surface, not
a curve), so Faltings' theorem is not directly applicable.  The rational
curve $t \mapsto (t^4, t^5, t^{10})$ has genus~$0$, which is
consistent with the existence of infinitely many rational points on it,
but this is consistency, not a proof.
...
\begin{theorem}[Proved results]
  The Diophantine equation $x^5 + y^4 = 2z^2$ has infinitely many integer
  solutions.  Three explicit parametric families are verified:
  \begin{enumerate}[label=\emph{(\roman*)}]
    \item $(t^4, t^5, t^{10})$ for $t \in \Z$;
    \item $(2m^2, 0, 4m^5)$ for $m \in \Z$;
    \item $(-k^4, k^5, 0)$ for $k \in \Z$.
  \end{enumerate}
  Families~(ii) and~(iii) are proved to be non-primitive for all parameters
  with $|$parameter$| \geq 2$.  Family~(i) is primitive if and only if $|t|=1$.
\end{theorem}

\begin{openquestion}
  Are $(1, 1, 1)$ and $(-1, 1, 0)$ (up to the symmetries $y \to -y$,
  $z \to -z$) the only primitive solutions?  A computational search up
  to $|x|, |y| \leq 500$ finds no others, but this does not constitute
  a proof.  A complete classification likely requires a descent argument
  or arithmetic-geometric methods.
\end{openquestion}
...
\midrule
Seed solution $(1,1,1)$ verified & \checkmark\ Proved & Direct substitution \\
Family 1 $(t^4,t^5,t^{10})$ verified & \checkmark\ Proved & \texttt{ring} identity \\
Family 2 $(2m^2,0,4m^5)$ verified & \checkmark\ Proved & \texttt{ring} identity \\
Family 3 $(-k^4,k^5,0)$ verified & \checkmark\ Proved & Unique factorisation + \texttt{ring} \\
Infinitely many distinct solutions & \checkmark\ Proved & Strict monotonicity of $n^4$ \\
Families 2, 3 never primitive for $|$param$|\geq 2$ & \checkmark\ Proved & Direct gcd computation \\
Family 1 primitive iff $|t|=1$ & \checkmark\ Proved & Direct gcd computation \\
Lean~4 formalisation (0 sorry) & \checkmark\ Proved & Mathlib tactics \\
Primitive solutions outside search bound & Open (Conjecture~\ref{conj:prim}) & Requires descent/AG methods \\
Classification of all primitive solutions & Open & Arithmetic geometry \\
\bottomrule
\end{tabular}
\end{table}
