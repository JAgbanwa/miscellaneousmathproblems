...
### Primitivity of each family (proved)
...
These are **proved** results. The Lean file contains formal proofs of all three.

### Computational search (evidence only)

> ⚠️ **Warning:** A finite search is not a proof.  The following is computational evidence, not a theorem.
...
No other primitive solutions were found within the search range.  This supports, but **does not prove**, the conjecture below.

### Conjecture (open problem)

**Conjecture:** Up to the symmetries $y \to -y$ and $z \to -z$, the only primitive solutions are $(1, 1, 1)$ (Seed A) and $(-1, 1, 0)$ (Seed B).

Proving this would require one of:
- A descent argument in $\mathbb{Z}$,
- A factorisation argument in a suitable ring (e.g. $\mathbb{Z}[\sqrt{2}]$),
- A height-bound argument from arithmetic geometry.

This is an **open problem** and is not claimed proved anywhere in this repository.
...
## Structure of the Solution Set (Motivational)

> ⚠️ The following is heuristic motivation, not a rigorous argument.  No result in this repository depends on these claims.

The equation $x^5 + y^4 = 2z^2$ is weighted-homogeneous of degree 20 with weight vector $(4,5,10)$.  This motivates the parametric families: any solution scales to another by $(x,y,z) \mapsto (t^4 x, t^5 y, t^{10} z)$.  The language of weighted projective space $\mathbb{P}^2_{(4,5,10)}$ describes this scaling, but we do not invoke any theorem about integral or rational points on weighted projective varieties.

Faltings' theorem concerns smooth projective curves of genus $\geq 2$ and is not applicable here (the equation defines a surface, not a curve; the parametric family sweeps out a rational curve of genus 0 within it).
