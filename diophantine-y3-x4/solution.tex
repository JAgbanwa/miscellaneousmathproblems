...
SingularPoints(C);          // returns empty set
assert GeometricGenus(C) eq 3;

// Bounded rational point search (height <= 1000)
pts := PointSearch(C, 1000);
print pts;   // { (0 : 1 : 0) }

// Chabauty-Coleman at p = 7
J := Jacobian(C);
rank := Rank(J);          // verified <= 2
rat_pts := Chabauty(C);   // Coleman integration at p = 7
print rat_pts;             // { (0 : 1 : 0) }
\end{lstlisting}

\textbf{Output:} \texttt{rank = 2}, rational points $= \{(0:1:0)\}$.
...
\bibitem{chabauty1941}
  C.~Chabauty,
  \textit{Sur les points rationnels des vari\'et\'es alg\'ebriques dont
    l'irr\'egularit\'e est sup\'erieure \`a la dimension},
  C.~R.~Acad.~Sci.~Paris \textbf{212} (1941), 1022--1024.
