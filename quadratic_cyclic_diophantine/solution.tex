...
\begin{lemma}[Equal coordinates]\label{lem:equal}
  \begin{enumlist}
    \item[(i)] $f(a,a,a) = 3a^3$.
    \item[(ii)] $f(a,a,b) = a^3 + a^2b + b^2a = a^2(a+b) + ab^2$.
    \item[(iii)] $f(a,b,a) = a^2b + b^2a + a^3 = a^2(a+b) + ab^2$ as well
          (same as (ii) by a relabelling).
    \item[(iv)] $f(0,b,c) = b^2c$ and $f(a,0,c) = c^2a$,
          $f(a,b,0) = a^2b$.
  \end{enumlist}
\end{lemma}
...
theorem solutions_zproj_infinite
    : (solutions.image (·.2.2)).Infinite := by
...
/-- Residual: for N ≥ 11395 a solution with z > N exists.
    No sorry-free proof is currently available; see §8.3. -/
